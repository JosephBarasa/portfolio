...
const useFetch = () => {
    ...
    useEffect(() => {
        ...
        const fetchAllData = async () => {
            try {
                ...
                const [softwareDevRes, visualArtsRes] = await Promise.all([
                    fetch(`${API_BASE_URL}/softwareDev`, { signal: abortCont.signal }),
                    fetch(`${API_BASE_URL}/visualArts`, { signal: abortCont.signal })
                ]);

                // check for specific fetch errors for softwareDev projects
                if (!softwareDevRes.ok) {
                    throw Error(`Software Development : Could not fetch data (Status : ${softwareDevRes.status}).`);
                }
        ...
        fetchAllData(); // execute the async fetch function

        // cleanup func: aborts the fetch request if the component unmounts
